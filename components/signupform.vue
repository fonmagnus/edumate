<template lang="pug">
	v-card
		...
		v-card-text
			...
			v-layout(row wrap)
				v-flex.xs12.pl-3.pr-3
					v-text-field(v-model="password2", color="orange accent-3" label="Confirm password", prepend-icon="lock", type="password")
...
export default {
    ...
    methods: {
        ...
        validateUser(){
            if(this.username.includes("mail.com") && this.password === this.password2 && this.password.length > 0){
				this.isValidMail = true;
				this.isValidPass = true;
				this.isValid = true;
                this.password = '';
